...
class BoardEvaluator(object):
	...
	def __init__ (self):
		...
		# add weight of 3 to the center, 2 to the outer square, then
		# 1, at last 0 to the outermost square.
		self.POS = []
		for i in range(7):
			row = []
			for j in range(7):
		...
		self.result = [ 0 for i in range(14) ]		# save current reslut of analyzation in a line
		self.line = [ 0 for i in range(14) ]		# current data in a line
	...
	def __analysis_horizon (self, board, i, j):
		...
		for x in range(7):
			line[x] = board[i][x]
		self.analysis_line(line, result, 7, j)
		for x in range(7):
	...
	def __analysis_vertical (self, board, i, j):
		...
		for x in range(7):
			line[x] = board[x][j]
		self.analysis_line(line, result, 7, i)
		for x in range(7):
			if result[x] != unanalyzed:
