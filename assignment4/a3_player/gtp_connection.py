...
class GtpConnection():
    ...
    def genmove_cmd(self, args):
        ...
        # select by random policy
        # cboard = self.board.copy()
        # _, moves = PatternUtil.generate_policy_moves(cboard)
        # select move by simulation
        # move = self.go_engine.select_move(self.board, self.board.current_player, moves)
        
        # instead of using rule base (default simulation == rule_based)
        cboard = self.board.copy()
        move = self.go_engine.get_move(cboard, color)
    ...
    def policy_cmd(self, args):
        ...
        if policy_type not in ["random", "rule_based"]:
            self.respond("illegal policy {}".format(policy_type))
        else:
